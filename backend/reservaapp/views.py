from reservaapp.serializers import AgendaSerializer, Centro_com_espacosSerializer, CentroEsportivoSerializer, CustomUserSerializer, DashboardGerenteSerializer, EspacoEsportivoSerializer, HorarioDisponivelSerializer, ReservaDetalhadaSerializer, ReservaSerializer, PagamentoSerializer, AgendaDetalhadaSerializer
...
from .filters import CentroEsportivoFilter, EspacoEsportivoFilter, AgendaFilter, DashboardFilter
...
class EspacoEsportivoViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.tipo != 'gerente' or centro_esportivo.gerente != self.request.user:
            raise PermissionError("Apenas o gerente do centro esportivo pode adicionar espaços esportivos.")      
        serializer.save()
...
class AgendaViewSet (viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.tipo != 'gerente' or espaco.centro_esportivo.gerente != self.request.user:
            raise PermissionError("Apenas o gerente do centro esportivo pode adicionar horários na agenda.")      
        serializer.save()
...
class HorariosDisponiveisView(APIView):
    def get(self, request, espaco_id):
        ...
        if not dia_str:
            return Response({'error': 'Parâmetro "dia" é obrigatório.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'error': 'Espaço esportivo não encontrado.'}, status=status.HTTP_404_NOT_FOUND)
        ...
        for agenda in agendas_disponiveis:
            ...
            horarios_categorizados[categoria].append(HorarioDisponivelSerializer(agenda).data)
...
class MeuCentroEsportivoView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = CentroEsportivoSerializer(centros, many=True, context={'request': request})
...
class EstatisticasGerenteView(APIView):
    ...
    def get(self, request):
        ...
        stats_status = reservas.values('status').annotate(
            total=Count('id')
        ).order_by('-total')
        
        espacos_populares = reservas.values(
            'agenda__espacoesportivo__nome',
            'agenda__espacoesportivo__categoria'
        ).annotate(
            total_reservas=Count('id')
        ).order_by('-total_reservas')[:5]
        
        horarios_populares = reservas.values(
            'agenda__h_inicial',
            'agenda__h_final'
        ).annotate(
            total_reservas=Count('id')
        ...
        categorias_populares = reservas.values(
            'agenda__espacoesportivo__categoria'
        ).annotate(
            total_reservas=Count('id')
        ).order_by('-total_reservas')

        arrecadacao_total = sum(reserva.agenda.preco for reserva in reservas if reserva.status == 'pago')

        reservas_concluidas = reservas.filter(status='pago').values(
            'agenda__espacoesportivo__nome',
            'agenda__dia',
...
class PagamentoViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if valor_pago < (valor_total_reserva / 2):
            raise ValidationError(f"O pagamento mínimo é de 50% do valor total (R$ {valor_total_reserva / 2:.2f}).")
